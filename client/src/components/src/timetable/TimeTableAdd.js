  ...
  changeFileHandler = e => {
   ...
   submitHandler = async e => {
    ...
    if (this.state.file.type.split('/')[0] === 'application') {
      ...
      const uploadTask = storage.ref(`/timeTable/${this.state.fileName}`).put(this.state.file)
      ...
      async (complete) => {
        try {
          const url = await storage.ref('timeTable').child(this.state.fileName).getDownloadURL()
            formData.append('url', url)
          const res = await axios.post('/timetable/addTimetable', formData, {
            headers : {
          ...
          if(this.state.uploadPercentage === 100){
            this.setState({message: 'Timetable Added!'})
          }
