...
const TimeTableContent = (props) => {
  ...
  const deleteHandler = () => {
    axios.delete(`/timetable/deleteTimetable/${props.id}&${props.file}`)
    // axios.delete(`/budget/deleteBudget/${props.id}`)
    .then(()=> { 
      const deleteRef = storage.ref().child(`timeTable/${props.file}`)
...
<Col md={6} lg={4}>
    <Card style={{minHeight: "360px", margin:"10px",  boxShadow: '0px 2px 10px rgba(0,0,0,0.15)'}}>
        ...
        <img className="card-img-top" style={{objectFit: "cover"}} src={require('../../../assets/timetable/file.png')} alt="Card"/>
