  ...
  changeFileHandler = e => {
   ...
   submitHandler = async e => {
    ...
    if (this.state.file.type.split('/')[0] === 'application') {
      ...
      const uploadTask = storage.ref(`/handout/${this.state.fileName}`).put(this.state.file)
      ...
      async (complete) => {
        try {
          const url = await storage.ref('handout').child(this.state.fileName).getDownloadURL()
