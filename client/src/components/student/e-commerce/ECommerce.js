...
const ECommerce = () => {
	...
	useEffect(() => {
		if (query !== null) {
			axios.get(`/ecommerce/searchEcommerce/${query}`).then((res) => {
				setItems(res.data);
				setLoading(false);
			});
		} else {
	...
	const productHandler = () => {
		const type = 'product';
		axios
			.get(`/ecommerce/typeEcommerce/${type}`)
			.then((res) => {
				setItems(res.data);
				setLoading(false);
			})
	...
	return (
		<div className='wrapper'>
			<Container>
				...
				<div className='mt-3'>
					<Button variant='info' className='m-2' onClick={productHandler}>
						#product
					</Button>
					<Button variant='info' className='m-2' onClick={advertisementHandler}>
						#advertisement
					</Button>
				</div>
				<Row
